...
class ProblemQ(Problem):
    ...
    def reset_init(self):
        ...
        # self.x = nn.Parameter(1+1*2*(torch.rand(self.d_x,1)-0.5),requires_grad=True)
        # self.dual_y = nn.Parameter(1+1*2*(torch.rand(self.d_y,1)-0.5), requires_grad=True)
...
class ProblemDRO(Problem):
    ...
    def estimate_var(self, data, target, b, sim_times = 100):
        full_batch = torch.arange(data.shape[0]).to(self.device)
        loss1 = self.loss(data, full_batch, target)
        loss1.backward()
        gx = [param.grad.data.clone() for name,param in self.named_parameters() if name!='dual_y']
        gy = [param.grad.data.clone() for name,param in self.named_parameters() if name=='dual_y']
        self.zero_grad()
        ...
        for sim in range(sim_times):
            data_loader_dumb = torch.randperm(data.shape[0]).to(self.device)
            batch_index = data_loader_dumb[:b]
            data_by_batch = torch.index_select(data,0,index=batch_index) #unseueeze is to make [64,28,28] to [64,1,28,28]
            target_by_batch = torch.index_select(target,0,index=batch_index)
            loss2 = self.loss(data_by_batch, batch_index, target_by_batch)
            loss2.backward()
            sto_gx = [param.grad.data.clone() for name,param in self.named_parameters() if name!='dual_y']
            sto_gy = [param.grad.data.clone() for name,param in self.named_parameters() if name=='dual_y']
            self.zero_grad()

            delta_x[sim] = sum([torch.norm(gx[i] - sto_gx[i])**2 for i in range(len(gx))]).item()
            delta_y[sim] = sum([torch.norm(gy[i] - sto_gy[i])**2 for i in range(len(gy))]).item()
        #print(f'var_x:{delta_x}')
        #print(f'var_y:{delta_y}')
        print(f'stdx:{np.sqrt(sum(delta_x)/sim_times)}, stdy:{np.sqrt(sum(delta_y)/sim_times)}')
        return np.sqrt(sum(delta_x)/sim_times) , np.sqrt(sum(delta_y)/sim_times)
