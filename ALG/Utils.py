...
def projection_simplex_sort2(v, z=0.99):
    # if np.isinf(v).any():
    #     return v
    # return np.minimum(np.maximum(v, -10),10)
...
def comlementdata(x,y):
    import bisect
    xlimit = min([ele[-1] for ele in x])
    n = max([len(ele) for ele in x] )

    xvalue = [i*xlimit/(n-1) for i in range(n)]
    yvalue = []
    for i in range(len(x)):
        cur_y = []
        for point in xvalue:
            idx = bisect.bisect_left(x[i],point)

            if x[i][idx] == point:
                cur_y.append(y[i][idx])
                continue
            elif idx == 0:
                idxl,idxr = 0,1
            elif idx == len(x[i]):
                idxl,idxr = len(x[i])-2,len(x[i])-1
            else:
                idxl,idxr = idx-1,idx
            Lambda = (point-x[i][idxr])/(x[i][idxl]-x[i][idxr])
            cur_y.append(Lambda*y[i][idxl] + (1-Lambda)*y[i][idxr])
        
        yvalue.append(copy.deepcopy(cur_y))
...
def shadowplot(x: list, y: list, alg_name: str, center=0, alpha=0.5, label_input=None, is_var=False,is_log=False,is_step = False,is_speical=False, plot_part=None):
    ...
    colors = {'GS-GDA-B,N=1': 'b',
              ...
              'GS-GDA-B,N=3': 'y',
              'GS-GDA-B,N=2': 'y',
              'primal_line_search_100': 'saddlebrown',\
              'LS-GS-GDA':'r', 'LS-GS-GDA-S':'b', 'LS-GS-GDA-R':'lightgreen', 'LS-GS-GDA-S-R':'r', \
              'J-GDA': 'saddlebrown', 
              'GS-GDA': 'lightgreen', 
              'TiAda': 'c', \
              'Smooth-AGDA':'m',
    ...
    linestyle_input = {'GS-GDA-B,N=1': '-', 'J-GDA':'-','GS-GDA':'-' ,'TiAda':'-',\
                       'GS-GDA-B,N=5': '-','GS-GDA-B,N=3': '-','GS-GDA-B,N=2': '-','GS-GDA-B,N=100': '-', \
    ...
    if is_speical:
        ...
        label = {'TiAda':'TiAda','J-GDA': 'GDA', 'GS-GDA': 'AGDA','Smooth-AGDA':'Sm-AGDA',\
            'LS-GS-GDA':'AGDA+','LS-GS-GDA-S': 'AGDA+S','LS-GS-GDA-R': 'RAGDA+','LS-GS-GDA-S-R': 'ADGA+SR','VRLM':'VRLM',\
            'GS-GDA-B,N=1': 'SGDA-B,N=1',
            'GS-GDA-B,N=5': 'SGDA-B,N=5',
            'GS-GDA-B,N=3': 'SGDA-B,N=3',
            'GS-GDA-B,N=2': 'SGDA-B,N=2',
                       }
    ...
    if 0:#plot_part =='z':
        mid_line = smooth_data(mid_line,500)
        lowline = smooth_data(lowline,500)
        highline = smooth_data(highline,500)
...
def most_frequent_id(id_list):
    # 计算每个元素出现的次数
    counts = Counter(id_list)
    # 返回出现次数最多的元素
    # most_common(1) 返回一个列表，列表中包含一个元组，元组的第一个元素是出现次数最多的元素，第二个元素是该元素出现的次数。
    # [0][0] 是为了获取这个元素的值
...
def computeGrad(model:Problem, data_by_batch,target_by_batch, batch_index, b, isOnestepmethod = False):
    model.zero_grad()
    if model.name[0] == 'Q':
