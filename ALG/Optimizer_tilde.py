...
class ALG():
    def __init__(self, train_set, data_name, mu_y, kappa = 10,
        ...
        if model_type == 'Q':
            ...
            self.estimated_L = estimated_L # max eigenvalue of Q
            self.estimated_mu = estimated_mu # min eigenvalue of Q
        else:
    ...
    def line_search(self, gamma:float=0.9, N:int=1, T=3, method:str=None, min_b:int=1, force_b:int=-1, kernal='AGDA', randompick=False):
        ...
        for sim in range(self.sim_time): 
            ...
            while not find:
                ...
                for t in range(T):
                    ...
                    self.record['config'][s] = {'lr_x':lr_x,'lr_y':lr_y,'b':b,'eps':eps,'N':N,'K':self.max_iter,'real_K':max_iter,'L':L,'mu_y':self.mu_y,'mu':mu.detach().cpu().numpy(),'std_x':self.start_model.std_x, 'std_y':self.start_model.std_y,'is_force_b':force_b>=1}
        ...
        if self.is_save_data:
            if self.model_type == 'Q':
                ...
                save_kappa = 1
            import os
            folder_path = './result_data/tilde' + foo + '_muy_' + str(self.mu_y) + '_kappa_' + str(save_kappa) + '_b_' + str(self.b)
