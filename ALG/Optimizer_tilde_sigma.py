...
class ALG():
    def __init__(self, train_set, data_name, mu_y, kappa = 10,
        ...
        if model_type == 'Q':
            estimated_L, estimated_mu = estimate_L_mu(self.start_model)
            geo_mean = torch.sqrt(estimated_L * estimated_mu)
    ...
    def line_search(self, gamma:float=0.9, xi=0.5, alpha=0.5, N:int=1, T=3, method:str=None, min_b:int=1, force_b:int=-1, kernal='AGDA', randompick=False):
        ...
        for sim in range(self.sim_time): 
            ...
            while not find:
                ...
                print(f'Contraction {s}, L: {L}, mu: {mu}, sigma_x: {sigma_x}, sigma_y: {sigma_y}')
                lr_y = 1 / L
                rho = ((1+12/N)**(1/2)-1)/24
                lr_x = N*rho*mu**2*lr_y**3
                ...
                for t in range(T):
                    ...
                    eps = 64/self.max_iter/lr_y**3/mu*(max(0,Phi0 - self.start_model.F_lower)/rho/mu + 6*delta0) # compute the precison we can get according to iteration number
                    eps = eps**(1/2)
                    ...
                    self.record['config'][s] = {'lr_x':lr_x,'lr_y':lr_y,'b':b,'eps':eps,'N':N,'K':self.max_iter,'real_K':max_iter,'L':L,'mu_y':self.mu_y,'mu':mu.item() if self.model_type == 'Q' else mu,'std_x':self.start_model.std_x, 'std_y':self.start_model.std_y,'is_force_b':force_b>=1}
                ...
                if not find:
                    ...
                    # rcondition = 4*N/lr_x*(max(0,Phi0 - Phik) + 6*rho*mu*delta0) \
                    #     + 4*len(self.record['norm_square_sto_grad_x'][s])/b*(self.model_curr.std**2
                    #                                                         +(1+6*(2-lr_y*mu)/lr_y/mu/(1-lr_y*mu)*self.model_curr.std**2)
                    #                                                         )
                    rcondition = 4*N/lr_x*(max(0,Phi0 - Phik) + 6*rho*mu*delta0) \
                        + 4*len(self.record['norm_square_sto_grad_x'][s])/b*(sigma_x**2
                                                                            +(1+6*(2-lr_y*mu)/lr_y/mu/(1-lr_y*mu if lr_y*mu!=1 else 1e-6)*sigma_y**2)
                                                                            )
    ...
    def maximizer_solver(self,start,lr_y=None,b=None,tol=None,max_iter = 1e5):
        ...
        while iter<int(max_iter):
            ...
            if self.model_type == 'DRO' and 0:
                ...
                y = model_tmp.exact_y_opt(None, None, None).clone()
                if self.projection_y:
                    y = torch.tensor(pj_y(y.cpu().detach().numpy()), device=self.device)
                return y
    ...
    def prepare_batch(self, model, b):
        full_batch = torch.arange(self.total_number_data).to(self.device)
        data_loader_dumb = self.batchselect()
        #initialize the counters for this contraction
        batch_start = 0
        if batch_start+b <= len(data_loader_dumb):
            ...
            # data_loader_dumb = self.batchselect()
            # batch_start = 0
            raise Exception('Unknown error. Should not happen')

        data_by_batch = torch.index_select(self.data,0,index=batch_index) #unseueeze is to make [64,28,28] to [64,1,28,28]
        target_by_batch = torch.index_select(self.targets,0,index=batch_index)
        return data_by_batch, target_by_batch, batch_index
